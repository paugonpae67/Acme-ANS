...
public interface TechnicianDashboardRepository extends AbstractRepository {
	...
	@Query("select avg(mr.estimatedCost.amount) from MaintenanceRecord mr where mr.technician.id = ?1 and mr.estimatedCost.currency =?3 and mr.draftMode = false and YEAR(mr.maintenanceMoment) = ?2")
	Double findAverageEstimatedCostLastYear(int technicianId, int currentYear, String currency);

	@Query("select min(mr.estimatedCost.amount) from MaintenanceRecord mr where mr.technician.id = ?1 and mr.estimatedCost.currency =?3 and mr.draftMode = false  and YEAR(mr.maintenanceMoment) = ?2")
	Double findMinimumEstimatedCostLastYear(int technicianId, int currentYear, String currency);

	@Query("select max(mr.estimatedCost.amount) from MaintenanceRecord mr where mr.technician.id = ?1 and mr.estimatedCost.currency =?3 and mr.draftMode = false  and YEAR(mr.maintenanceMoment) = ?2")
	Double findMaximumEstimatedCostLastYear(int technicianId, int currentYear, String currency);

	@Query("select stddev(mr.estimatedCost.amount) from MaintenanceRecord mr where mr.technician.id = ?1 and mr.estimatedCost.currency =?3 and mr.draftMode = false and YEAR(mr.maintenanceMoment) = ?2")
